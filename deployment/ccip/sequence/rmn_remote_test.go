...
func TestAptosCurseSequence_Success(t *testing.T) {
	...
	var gotInput rmnops.CurseMultipleInput
	stubTx := mcmstypes.Transaction{}

	rmnops.CurseMultipleOp = cldf_ops.NewOperation(
		"stub-curse-op",
		operation.Version1_0_0,
		"stub",
		func(_ cldf_ops.Bundle, deps dependency.AptosDeps, in rmnops.CurseMultipleInput) (mcmstypes.Transaction, error) {
			gotDeps = deps
			gotInput = in
			return stubTx, nil
		},
...
func TestAptosUncurseSequence_Success(t *testing.T) {
	...
	var gotDeps dependency.AptosDeps
	var gotInput rmnops.UncurseMultipleInput
	stubTx := mcmstypes.Transaction{}

	rmnops.UncurseMultipleOp = cldf_ops.NewOperation(
		"stub-uncurse-op",
		operation.Version1_0_0,
		"stub",
		func(_ cldf_ops.Bundle, deps dependency.AptosDeps, in rmnops.UncurseMultipleInput) (mcmstypes.Transaction, error) {
			gotDeps = deps
			gotInput = in
			return stubTx, nil
		},
...
func overrideOps() func() {
	origCurse := rmnops.CurseMultipleOp
	origUncurse := rmnops.UncurseMultipleOp
	return func() {
		rmnops.CurseMultipleOp = origCurse
		rmnops.UncurseMultipleOp = origUncurse
	}
}
