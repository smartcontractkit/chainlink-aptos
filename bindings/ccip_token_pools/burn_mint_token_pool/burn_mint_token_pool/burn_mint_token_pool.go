...
type BurnMintTokenPoolEncoder interface {
	...
	InitializeTokenPoolEvents() (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error)
	Transfer(to aptos.AccountAddress, amount uint64) (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error)
...
type BurnMintTokenPoolEvents struct {
}
...
func (c BurnMintTokenPoolContract) ExecuteOwnershipTransfer(opts *bind.TransactOpts, to aptos.AccountAddress) (*api.PendingTransaction, error) {
	module, function, typeTags, args, err := c.burnMintTokenPoolEncoder.ExecuteOwnershipTransfer(to)
	if err != nil {
		return nil, err
	}
...
func (c burnMintTokenPoolEncoder) InitializeTokenPoolEvents() (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error) {
	return c.BoundContract.Encode("initialize_token_pool_events", nil, []string{}, []any{})
}

func (c burnMintTokenPoolEncoder) Transfer(to aptos.AccountAddress, amount uint64) (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error) {
	return c.BoundContract.Encode("transfer", nil, []string{
		"address",
		"u64",
	}, []any{
		to,
