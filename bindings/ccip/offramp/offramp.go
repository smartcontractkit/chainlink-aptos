...
type OfframpEncoder interface {
	...
	Commit(reportContext [][]byte, report []byte, rs [][]byte, ss [][]byte, vs []byte) (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error)
...
func (c OfframpContract) Commit(opts *bind.TransactOpts, reportContext [][]byte, report []byte, rs [][]byte, ss [][]byte, vs []byte) (*api.PendingTransaction, error) {
	module, function, typeTags, args, err := c.offrampEncoder.Commit(reportContext, report, rs, ss, vs)
	if err != nil {
		return nil, err
	}
...
func (c offrampEncoder) Commit(reportContext [][]byte, report []byte, rs [][]byte, ss [][]byte, vs []byte) (bind.ModuleInformation, string, []aptos.TypeTag, [][]byte, error) {
