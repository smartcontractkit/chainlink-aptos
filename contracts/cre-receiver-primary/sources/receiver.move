module cre_canary_receiver_primary::canary_receiver {
    use std::event;
    use std::option;
    use std::signer;
    ...
    fun init_module(publisher: &signer) {
        assert!(signer::address_of(publisher) == @cre_canary_receiver_primary, 1);
    ...
    fun handle_report(data: vector<u8>): option::Option<u128> acquires State {
        ...
        let state = borrow_global_mut<State>(@cre_canary_receiver_primary);
    ...
    public fun get_counter(): u64 acquires State {
        borrow_global<State>(@cre_canary_receiver_primary).counter
    }
}
