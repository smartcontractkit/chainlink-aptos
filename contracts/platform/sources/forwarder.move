...
module chainlink_platform::forwarder {
    ...
    public entry fun set_config(
        ...
        if (table::contains(&state.configs, config_id)) {
            let config = table::remove(&mut state.configs, config_id);
            config.f = f;
            config.oracles = oracles;
            table::add(&mut state.configs, config_id, config);
        } else {
            table::add(
                &mut state.configs,
                config_id,
    ...
    public fun signature_from_bytes(bytes: vector<u8>): Signature {
        ...
        let public_key = vector::slice(&bytes, 0, 32);
        let sig = vector::slice(&bytes, 32, 96);
    ...
    fun validate_and_process_report(
        ...
        let don_id_bytes = vector::slice(&report, 37, 41);
        let don_id = to_u32be(don_id_bytes);
        
        let config_version_bytes = vector::slice(&report, 41, 45);
        let config_version = to_u32be(config_version_bytes);
        
        let report_id_bytes = vector::slice(&report, 107, 109);
        let report_id = to_u16be(report_id_bytes);
    ...
    // Helper functions for parsing big-endian values
    fun to_u16be(data: vector<u8>): u16 {
        // Implementation would convert big-endian bytes to u16
        0 // Placeholder
    }
    
    fun to_u32be(data: vector<u8>): u32 {
        // Implementation would convert big-endian bytes to u32
        0 // Placeholder
    }
