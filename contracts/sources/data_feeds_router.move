module chainlink::data_feeds_router {
    ...
    use chainlink::data_feeds_registry::{Self, BenchmarkResult, ReportResult};
    ...
    fun assert_is_owner(router: &DataFeedsRouter, target_address: address) {
    ...
    fun get_registry_to_feed_ids(router: &DataFeedsRouter, feed_ids: &vector<vector<u8>>): SimpleMap<address, vector<vector<u8>>> {
    ...
    public fun get_benchmarks(_account: &signer, router_address: address, feed_ids: vector<vector<u8>>, _billing_data: vector<u8>): vector<BenchmarkResult> acquires DataFeedsRouter {
        // TODO: handle billing

        let router = borrow_global<DataFeedsRouter>(router_address);

        get_benchmarks_internal(router, feed_ids)
    }

    public fun get_benchmarks_nonbillable(router_address: address, feed_ids: vector<vector<u8>>, _cap: &NonbillableAccessCapability): vector<BenchmarkResult> acquires DataFeedsRouter {
        let router = borrow_global<DataFeedsRouter>(router_address);

        get_benchmarks_internal(router, feed_ids)
    }

    fun get_benchmarks_internal(router: &DataFeedsRouter, feed_ids: vector<vector<u8>>): vector<BenchmarkResult> {
        let router_signer = account::create_signer_with_capability(&router.signer_cap);
        ...
        vector::zip(registry_addresses, per_registry_feed_ids, |registry_address, registry_feed_ids| {
            let results = data_feeds_registry::get_benchmarks(&router_signer, registry_address, registry_feed_ids);
    ...
    public fun get_reports(_account: &signer, router_address: address, feed_ids: vector<vector<u8>>, _billing_data: vector<u8>): vector<ReportResult> acquires DataFeedsRouter {
        // TODO: handle billing
    ...
    public fun get_reports_nonbillable(router_address: address, feed_ids: vector<vector<u8>>, _cap: &NonbillableAccessCapability): vector<ReportResult> acquires DataFeedsRouter {
        let router = borrow_global<DataFeedsRouter>(router_address);

        get_reports_internal(router, feed_ids)
    }

    fun get_reports_internal(router: &DataFeedsRouter, feed_ids: vector<vector<u8>>): vector<ReportResult> {
        let router_signer = account::create_signer_with_capability(&router.signer_cap);
        ...
        let registry_to_results = simple_map::new();
        vector::zip(registry_addresses, per_registry_feed_ids, |registry_address, registry_feed_ids| {
            let results = data_feeds_registry::get_reports(&router_signer, registry_address, registry_feed_ids);
            simple_map::add(&mut registry_to_results, registry_address, results);
        });
    ...
    public fun get_descriptions(router_address: address, feed_ids: vector<vector<u8>>): vector<String> acquires DataFeedsRouter {
        let router = borrow_global<DataFeedsRouter>(router_address);
        ...
        let registry_to_results = simple_map::new();
        vector::zip(registry_addresses, per_registry_feed_ids, |registry_address, registry_feed_ids| {
            let results = data_feeds_registry::get_feed_metadata(registry_address, registry_feed_ids);
            simple_map::add(&mut registry_to_results, registry_address, results);
        });
        ...
        vector::for_each_reverse(feed_ids, |feed_id| {
            ...
            vector::push_back(&mut ret, data_feeds_registry::read_feed_metadata_description(&feed_metadata));
        });
    ...
    public entry fun configure_feeds(account: &signer, router_address: address, feed_ids: vector<vector<u8>>, descriptions: vector<String>, config_id: vector<u8>, upkeep: address, registry_address: address, _fee_config_id: vector<u8>) acquires DataFeedsRouter {
        // TODO: set new fee config
        let router = borrow_global<DataFeedsRouter>(router_address);
        assert_is_owner(router, signer::address_of(account));

        let router_signer = account::create_signer_with_capability(&router.signer_cap);
        data_feeds_registry::set_feeds(&router_signer, registry_address, feed_ids, descriptions, config_id, upkeep);

        set_registry(account, router_address, feed_ids, registry_address);
    }

    public entry fun set_registry(account: &signer, router_address: address, feed_ids: vector<vector<u8>>, registry_address: address) acquires DataFeedsRouter {
        let router = borrow_global_mut<DataFeedsRouter>(router_address);
        assert_is_owner(router, signer::address_of(account));
