...
module token_messenger_minter::token_messenger {
    ...
    public fun replace_deposit_for_burn(
        ...
        let mint_recipient =
            option::get_with_default(new_mint_recipient, old_mint_recipient);
        assert!(
            mint_recipient != @0x0,
            error::invalid_argument(EINVALID_MINT_RECIPIENT_ADDRESS)
        );
        let new_burn_message =
        ...
        event::emit(
            DepositForBurn {
                ...
                destination_token_messenger: message::get_recipient_address(
                    original_message
                ),
                destination_caller
            }
    ...
    public fun handle_receive_message(receipt: Receipt): bool {
        let (sender, recipient, remote_domain, message_body) =
            message_transmitter::get_receipt_details(&receipt);

        // Validate `recipient` in receipt is the `TokenMessengerMinter` contract
        assert!(
            recipient == state::get_object_address(),
            error::invalid_argument(ERECIPIENT_NOT_TOKEN_MESSENGER)
        );
        ...
        assert!(
            message_version == message_body_version(),
            error::invalid_argument(EINVALID_MESSAGE_BODY_VERSION)
        );
    ...
    entry fun add_remote_token_messenger(
        caller: &signer, domain: u32, token_messenger: address
    ) {
        ownable::assert_is_owner(caller, state::get_object_address());
        assert!(
            token_messenger != @0x0,
            error::invalid_argument(EINVALID_TOKEN_MESSENGER_ADDRESS)
        );
        assert!(
    ...
    fun deposit_for_burn_helper(
        ...
        let destination_token_messenger =
            state::get_remote_token_messenger(destination_domain);
        token_minter::burn(burn_token, asset);

        let serialized_burn_message =
            burn_message::serialize(
                message_body_version(),
                burn_token,
                mint_recipient,
    ...
    fun validate_remote_token_messenger(
    ...
    #[test_only]
    const REMOTE_TOKEN_MESSENGER: address =
        @0xe786e705b98581cbf28488ce4ae116db0918e1f7eb1877d07bf0995cf67724ef;
    #[test_only]
    ...
    fun get_valid_deposit_for_burn_message_and_attestation(): (vector<u8>, vector<u8>) {
        ...
        let original_attestation =
            x"278a9949e5e022256553a072ef00f9e5928235354c79f277539ec56174becea7133ac670f9fc6c38bca534e14aca05e8271112f5d8a9da3375c56d399790d5ea1c";
    ...
    fun get_valid_receive_message_and_attestation(): (vector<u8>, vector<u8>) {
        ...
        let attestation =
            x"2718d6be14108dfef8f0a24a5dc7a4623dd8139420c0b695baa8178cd2f48d8a70484a52ec5d71c9575685b0319927e669355139c099601d29e3f9363c2d20ba1c";
        ...
        assert!(
            event::was_event_emitted(
                &DepositForBurn {
                    ...
                    destination_token_messenger: from_bcs::to_address(
                        hash::sha3_256(b"token_messenger")
                    ),
                    destination_caller: @0x0
                }
            ),
    ...
    fun test_deposit_for_burn_message_transmitter_aborts(
        owner: &signer, mt_signer: &signer
    ) {
        init_test_token_messenger(owner);
    ...
    #[test(owner = @deployer)]
    #[expected_failure(abort_code = 0x1000a, location = Self)]
    fun test_deposit_for_burn_with_caller_invalid_destination_domain(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let amount = 12;
    ...
    #[test(owner = @deployer)]
    #[expected_failure(abort_code = 0x10002, location = Self)]
    fun test_deposit_for_burn_with_caller_invalid_mint_recipient(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let amount = 12;
    ...
    fun test_deposit_for_burn_with_caller_invalid_destination_caller(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let amount = 12;
    ...
    #[test(owner = @deployer, mt_signer = @message_transmitter)]
    #[expected_failure(abort_code = pausable::ENOT_PAUSER, location = pausable)]
    fun test_deposit_for_burn_with_caller_message_transmitter_aborts(
        owner: &signer, mt_signer: &signer
    ) {
        init_test_token_messenger(owner);
        mt_state::set_paused(mt_signer);
    ...
    #[test(owner = @deployer)]
    fun test_replace_deposit_for_burn_new_destination_caller(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        ...
        let (original_message, original_attestation) =
            get_valid_deposit_for_burn_message_and_attestation();
        ...
        let message_sent_event = vector::borrow(
            &event::emitted_events<MessageSent>(), 0
        );
        ...
        assert!(
            burn_message::get_version(&new_burn_message) == message_body_version(), 0
        );
    ...
    #[test(owner = @deployer)]
    fun test_replace_deposit_for_burn_new_destination_caller_version_bump(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let original_account_balance = get_signer_balance(owner);
    ...
    #[test(owner = @deployer)]
    fun test_replace_deposit_for_burn_new_mint_recipient_version_bump(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let new_mint_recipient = @0xfab;
    ...
    #[test(owner = @deployer)]
    fun test_replace_deposit_for_burn_new_mint_recipient_and_destination_caller(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let original_account_balance = get_signer_balance(owner);
    ...
    #[test(owner = @deployer)]
    #[expected_failure(abort_code = 0x10002, location = Self)]
    fun test_replace_deposit_for_burn_invalid_mint_recipient(
        owner: &signer
    ) {
        init_test_token_messenger(owner);
        let (original_message, original_attestation) =
    ...
    fun test_replace_deposit_for_burn_not_the_original_sender(
    ...
    fun test_add_remote_token_messenger(owner: &signer) {
        ...
        assert!(
            event::was_event_emitted(
                &RemoteTokenMessengerAdded { domain, token_messenger }
            ),
            0
        );
    }
    ...
    fun test_add_remote_token_messenger_not_owner(
        owner: &signer, not_owner: &signer
    ) {
        init_test_token_messenger(owner);
        let domain = 7;
    ...
    fun test_remove_remote_token_messenger(owner: &signer) {
        ...
        assert!(
            event::was_event_emitted(
                &RemoteTokenMessengerRemoved { domain, token_messenger }
            ),
            0
        );
    }
    ...
    #[test(owner = @deployer, not_owner = @0xfaa)]
    #[expected_failure(abort_code = ownable::ENOT_OWNER, location = ownable)]
    fun test_remove_remote_token_messenger_not_owner(
        owner: &signer, not_owner: &signer
    ) {
        init_test_token_messenger(owner);
        let domain = 7;
    ...
    #[
        test(
            owner = @deployer,
            destination_caller =
            @0x8a4edb71919e22728ffe9a925df33202328aca8d6e13be2c5f4e02b4370c8f71
        )
    ]
    fun test_handle_receive_message_success(
        ...
        let receipt =
            message_transmitter::receive_message(
                destination_caller, &message, &attestation
            );
        ...
        let expected_account_balance = account_balance + amount;
        assert!(
            token_minter::get_account_balance(mint_recipient)
    ...
    fun test_handle_receive_message_invalid_version(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let (message, attestation) = get_valid_receive_message_and_attestation();
    ...
    fun test_handle_receive_message_invalid_burn_message(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let message =
        ...
        let attestation =
            x"a17e6548171cc8d014ec1d3953e3b90ab17c47ae029de4fdf32bbfc68d7bdc5003a8e3fface57934d54e6f907a3b5fd5825da1ffec1016972d5a508f0738beda1c";
        let receipt =
            message_transmitter::receive_message(
                destination_caller, &message, &attestation
            );
    ...
    fun test_handle_receive_message_invalid_message_recipient(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let burn_message =
        ...
        let attestation =
            x"94e9bc648d05934214c52f5e8a43f44ab5bf74e4e19666ff00c7acb8d21b4bcc0f346dc91e953cf0bd09f89f77380480c01e4ce591b77ee0661cb3b1fa7f17361b";
        let receipt =
            message_transmitter::receive_message(
                destination_caller, &message, &attestation
            );
    ...
    #[expected_failure(abort_code = 0x10006, location = Self)]
    fun test_handle_receive_message_no_remote_token_messenger(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let (message, attestation) = get_valid_receive_message_and_attestation();
    ...
    fun test_handle_receive_message_invalid_remote_token_messenger(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let (message, attestation) = get_valid_receive_message_and_attestation();
    ...
    #[expected_failure(abort_code = 0x10001, location = Self)]
    fun test_handle_receive_invalid_amount(
        owner: &signer, destination_caller: &signer
    ) {
        init_test_token_messenger(owner);
        let message =
            x"";
        let attestation =
            x"543ac51d16dd8ccc8ba2cd849be3bc0ee63dfe490d01ad35a33017d904fbb3257a43b5f8699e817efc946fd99b4445a4b0864b0f763c152c57ac8600dcb388481b";
        let receipt =
            message_transmitter::receive_message(
                destination_caller, &message, &attestation
            );
    ...
    fun test_view_remote_token_messenger(owner: &signer) {
        init_test_token_messenger(owner);
        assert!(
            state::get_remote_token_messenger(REMOTE_DOMAIN)
                == remote_token_messenger(REMOTE_DOMAIN),
            0
        );
    }
    ...
    fun test_view_num_remote_token_messengers(owner: &signer) {
        init_test_token_messenger(owner);
        assert!(
            state::get_num_remote_token_messengers() == num_remote_token_messengers(),
            0
        );
    }
    ...
    fun test_view_max_burn_amount_per_message(owner: &signer) {
        ...
        let (_, max_burn_amount) =
            state::get_max_burn_limit_per_message_for_token(@stablecoin);
