...
module stablecoin::stablecoin_e2e_tests {
    ...
    fun mint_and_deposit__should_succeed() {
        ...
        treasury::test_configure_controller(
            &create_signer_for_test(MASTER_MINTER), CONTROLLER, MINTER
        );
    ...
    fun batch_mint_and_deposit__should_succeed() {
        ...
        let expected_mint_events = vector::singleton(
            treasury::test_Mint_event(MINTER, mint_allowance)
        );
        ...
        assert_eq(
            event::emitted_events<stablecoin::Deposit>(), expected_deposit_events
        );
    }
    ...
    fun withdraw_and_burn__should_succeed() {
        ...
        treasury::test_configure_minter(
            &create_signer_for_test(CONTROLLER), 10_000_000
        );
        ...
        // Mint and deposit to store.
        {
            ...
            assert_eq(
                fungible_asset::supply(stablecoin_metadata),
                option::some((10_000_000 as u128))
            );
        ...
        // Withdraw from store and burn.
        {
            let asset =
                dispatchable_fungible_asset::withdraw(
                    &create_signer_for_test(RANDOM_ADDRESS), store, 1_000_000
                );
            treasury::burn(&create_signer_for_test(MINTER), asset);

            assert_eq(
                fungible_asset::supply(stablecoin_metadata),
                option::some((9_000_000 as u128))
            );
            assert_eq(fungible_asset::balance(store), 9_000_000);

            let withdraw_event =
                stablecoin::test_Withdraw_event(
                    RANDOM_ADDRESS, /* store owner */
                    object::object_address(&store),
                    1_000_000
                );
