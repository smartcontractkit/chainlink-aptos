...
module stablecoin::fungible_asset_tests {
    ...
    fun setup__should_create_fungible_asset_with_metadata() {
        ...
        assert_eq(
            object::object_exists<fungible_asset::ConcurrentSupply>(fungible_asset_address),
            true
        );
        assert_eq(
            object::object_exists<fungible_asset::Metadata>(fungible_asset_address),
            true
        );
        assert_eq(
            object::object_exists<fungible_asset::Untransferable>(fungible_asset_address),
            true
        );
        assert_eq(
            object::object_exists<primary_fungible_store::DeriveRefPod>(
    ...
    fun deposit_with_ref__should_fail_if_ref_does_not_match_asset() {
        ...
        assert_eq(
            fungible_asset::store_metadata(store),
            fungible_asset::metadata_from_asset(&deposit_asset)
        );
    ...
    fun deposit_with_ref__concurrent_fungible_balance_disabled__should_fail_if_store_balance_overflow() {
        ...
        std::features::change_feature_flags_for_testing(
            ...
            vector[], /* enable */
            vector[
                std::features::get_default_to_concurrent_fungible_balance_feature()
            ] /* disable */
        );
        ...
        primary_fungible_store::mint(
            &fungible_asset::generate_mint_ref(&constructor_ref), HOLDER_ONE, U64_MAX
        );
    ...
    #[
        test,
        expected_failure(
            abort_code = ERR_AGGREGATOR_V2_EAGGREGATOR_OVERFLOW,
            location = aptos_framework::aggregator_v2
        )
    ]
    ...
    fun withdraw__should_succeed_if_caller_is_indirect_owner() {
        ...
        let object_address =
            object::address_from_constructor_ref(&object_constructor_ref);

        // Create a store which the object owns.
        let store = primary_fungible_store::create_primary_store(
            object_address, metadata
        );
        ...
        let withdrawn_asset =
            fungible_asset::withdraw(&create_signer_for_test(HOLDER_ONE), store, 10);
    ...
    fun withdraw__should_fail_if_amount_exceeds_store_balance() {
        ...
        let owner = create_signer_for_test(HOLDER_ONE);
        let withdrawn_asset = fungible_asset::withdraw(&owner, store, DEPOSIT_AMOUNT
            + 1);
    ...
    fun withdraw_with_ref__should_fail_if_ref_does_not_match_asset() {
        ...
        let withdrawn_asset =
            fungible_asset::withdraw_with_ref(&transfer_ref, store, DEPOSIT_AMOUNT);
    ...
    #[
        test,
        expected_failure(
            abort_code = ERR_FUNGIBLE_ASSET_EMAX_SUPPLY_EXCEEDED,
            location = aptos_framework::fungible_asset
        )
    ]
    fun mint__should_fail_if_exceed_max_total_supply() {
        let max_supply: u128 = ((U64_MAX as u128) + 100_000);
        let (constructor_ref, metadata, _) =
            setup_fa_with_max_supply(OWNER_ONE, option::some(max_supply));
        ...
        let asset =
            fungible_asset::mint(
                &fungible_asset::generate_mint_ref(&constructor_ref), 100_001
            );
    ...
    #[
        test,
        expected_failure(
            abort_code = ERR_INVALID_ARGUMENT_BURN_REF_AND_FUNGIBLE_ASSET_MISMATCH,
            location = aptos_framework::fungible_asset
        )
    ]
    ...
    fun setup_fa_with_max_supply(
        owner: address, max_supply: Option<u128>
    ): (ConstructorRef, Object<Metadata>, address) {
        let constructor_ref =
            object::create_named_object(
                &create_signer_for_test(owner), stablecoin_obj_seed()
            );
    ...
    public fun mint(constructor_ref: &ConstructorRef, amount: u64): FungibleAsset {
