module platform_a::forwarder {
    ...
    fun init_module(publisher: &signer) {
        assert!(signer::address_of(publisher) == @platform_a, 1);
    ...
    inline fun get_state_addr(): address {
        object::create_object_address(&@platform_a, APP_OBJECT_SEED)
    }
    ...
    fun dispatch(
        ...
        let meta = platform_a::storage::insert(receiver, metadata, data);
        ...
        assert!(
            !platform_a::storage::storage_exists(obj_address),
    ...
    #[test_only]
    public entry fun set_up_test(owner_a: &signer, publisher: &signer) {
        use aptos_framework::account::{Self};
        account::create_account_for_test(signer::address_of(owner_a));
    ...
    #[test(owner_a = @owner_a, publisher = @platform_a)]
    public entry fun test_happy_path(owner_a: &signer, publisher: &signer) acquires State {
        set_up_test(owner_a, publisher);
    ...
    #[test(owner_a = @owner_a, publisher = @platform_a, new_owner = @0xbeef)]
    fun test_transfer_ownership_success(
        owner_a: &signer, publisher: &signer, new_owner: &signer
    ) acquires State {
        set_up_test(owner_a, publisher);

        assert!(get_owner() == @owner_a, 1);

        transfer_ownership(owner_a, signer::address_of(new_owner));
        accept_ownership(new_owner);

        assert!(get_owner() == signer::address_of(new_owner), 2);
    }

    #[test(owner_a = @owner_a, publisher = @platform_a, unknown_user = @0xbeef)]
    #[expected_failure(abort_code = 327687, location = platform_a::forwarder)]
    fun test_transfer_ownership_failure_not_owner(
        owner_a: &signer, publisher: &signer, unknown_user: &signer
    ) acquires State {
        set_up_test(owner_a, publisher);
    ...
    #[test(owner_a = @owner_a, publisher = @platform_a)]
    #[expected_failure(abort_code = 65549, location = platform_a::forwarder)]
    fun test_transfer_ownership_failure_transfer_to_self(
        owner_a: &signer, publisher: &signer
    ) acquires State {
        set_up_test(owner_a, publisher);

        assert!(get_owner() == @owner_a, 1);

        transfer_ownership(owner_a, signer::address_of(owner_a));
    }

    #[test(owner_a = @owner_a, publisher = @platform_a, new_owner = @0xbeef)]
    #[expected_failure(abort_code = 327694, location = platform_a::forwarder)]
    fun test_transfer_ownership_failure_not_proposed_owner(
        ...
        transfer_ownership(owner_a, @0xfeeb);
        accept_ownership(new_owner);
    }
}
