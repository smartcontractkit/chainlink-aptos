...
/// with pool type `managed_token_pool`.
module ccip_offramp::managed_dispatchable_receiver {
    use std::account;
    use std::event;
    ...
    public fun type_and_version(): String {
        string::utf8(b"ManagedDispatchableReceiver 1.6.0")
    }
    ...
    const MODULE_NAME: vector<u8> = b"managed_dispatchable_receiver";
    ...
    public fun ccip_receive<T: key>(_metadata: Object<T>): Option<u128> acquires CCIPReceiverState {
        ...
        if (dest_token_amounts.length() != 0 && data.length() != 0) {
            ...
            for (i in 0..dest_token_amounts.length()) {
                ...
                managed_token_pool::transfer(&state_signer, final_recipient, amount);
            };
