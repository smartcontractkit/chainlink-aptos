...
module chainlink_mcms::mcms {
    ...
    fun create_multisig(role: u8, ctx: &mut TxContext): address {
        // Create a UID for the multisig address first
        let multisig_id = object::new(ctx);
        let multisig_address = object::uid_to_address(&multisig_id);
    ...
    public entry fun set_root(
        ...
        let _signed_hash = compute_eth_message_hash(root, valid_until);
        
        // Get the appropriate multisig object based on role
        let _multisig_addr = get_multisig_address(state, role);
    ...
    public entry fun execute(
        _state: &mut MultisigState,
        _timelock: &mut Timelock,
        role: u8,
        chain_id: u256,
        multisig_addr: address,
        nonce: u64,
    ...
    public fun compute_eth_message_hash(
        root: vector<u8>, _valid_until: u64
    ): vector<u8> {
