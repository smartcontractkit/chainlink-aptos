module ccip::ocr3_base {
    ...
    inline fun verify_signature(
        ...
        assert!(len == vector::length(&ss) && len == vector::length(&vs), error::invalid_argument(E_INVALID_SIGNATURE));
        ...
        for (i in 0..len) {
            ...
            let evm_address = vector::trim(&mut aptos_hash::keccak256(public_key_bytes), 12);
