module ccip::onramp {
  ...
  public fun initialize(
    ...
    apply_dest_chain_config_updates_unchecked(dest_chain_selectors, dest_chain_enabled, dest_chain_allowlist_enabled);
  }

  #[view]
  public fun get_expected_next_sequence_number(dest_chain_selector: u64): u64 acquires OnRampState {
    let state = borrow_state();
    assert!(smart_table::contains(&state.dest_chain_configs, dest_chain_selector), error::invalid_argument(E_UNKNOWN_DEST_CHAIN_SELECTOR));
    let dest_chain_config = smart_table::borrow(&state.dest_chain_configs, dest_chain_selector);
  ...
  public fun ccip_send(
    ...
    extra_args: vector<u8>): vector<u8> acquires OnRampState {

    let state = borrow_state_mut();
    assert!(smart_table::contains(&state.dest_chain_configs, dest_chain_selector), error::invalid_argument(E_UNKNOWN_DEST_CHAIN_SELECTOR));

    let dest_chain_config = smart_table::borrow_mut(&mut state.dest_chain_configs, dest_chain_selector);
    assert!(dest_chain_config.is_enabled, error::permission_denied(E_DEST_CHAIN_NOT_ENABLED));

    if (dest_chain_config.allowlist_enabled) {
      assert!(vector::contains(&dest_chain_config.allowed_senders, &signer::address_of(caller)), error::permission_denied(E_SENDER_NOT_ALLOWED));
    };
    ...
    let token_amounts = vector::map(token_transfers, |token_transfer| {
  ...
  public entry fun set_dynamic_config(caller: &signer, allowlist_admin: address) acquires OnRampState {
    ownable::assert_only_owner(signer::address_of(caller), &borrow_state().ownable_state);

    set_dynamic_config_unchecked(allowlist_admin)
  }

  public entry fun apply_dest_chain_config_updates(caller: &signer, dest_chain_selectors: vector<u64>, dest_chain_enabled: vector<bool>, dest_chain_allowlist_enabled: vector<bool>) acquires OnRampState {
    ownable::assert_only_owner(signer::address_of(caller), &borrow_state().ownable_state);
  ...
  public fun get_dest_chain_config(dest_chain_selector: u64): (bool, u64, bool) acquires OnRampState {
    ...
    (dest_chain_config.is_enabled, dest_chain_config.sequence_number, dest_chain_config.allowlist_enabled)
  }

  #[view]
  public fun get_allowed_senders_list(dest_chain_selector: u64): (bool, vector<address>) acquires OnRampState {
    let state = borrow_state();
  ...
  public entry fun apply_allowlist_updates(
    ...
    dest_chain_remove_allowed_senders: vector<vector<address>>) acquires OnRampState {
    let state = borrow_state_mut();
    assert!(signer::address_of(caller) == ownable::owner(&state.ownable_state) || signer::address_of(caller) == state.allowlist_admin, error::permission_denied(E_ONLY_CALLABLE_BY_OWNER_OR_ALLOWLIST_ADMIN));

    let dest_chains_len = vector::length(&dest_chain_selectors);
    assert!(dest_chains_len == vector::length(&dest_chain_allowlist_enabled), error::invalid_argument(E_DEST_CHAIN_ARGUMENT_MISMATCH));
    assert!(dest_chains_len == vector::length(&dest_chain_add_allowed_senders), error::invalid_argument(E_DEST_CHAIN_ARGUMENT_MISMATCH));
    assert!(dest_chains_len == vector::length(&dest_chain_remove_allowed_senders), error::invalid_argument(E_DEST_CHAIN_ARGUMENT_MISMATCH));
    ...
    while (i < dest_chains_len) {
      ...
      let add_allowed_senders = *vector::borrow(&dest_chain_add_allowed_senders, i);
      let remove_allowed_senders = *vector::borrow(&dest_chain_remove_allowed_senders, i);

      let dest_chain_config = smart_table::borrow_mut(&mut state.dest_chain_configs, dest_chain_selector);
      dest_chain_config.allowlist_enabled = allowlist_enabled;

      if (vector::length(&add_allowed_senders) > 0) {
        assert!(allowlist_enabled, error::invalid_argument(E_INVALID_ALLOWLIST_REQUEST));
        vector::for_each_ref(&add_allowed_senders, |sender_address| {
          let sender_address: address = *sender_address;
          assert!(sender_address != @0x0, error::invalid_argument(E_INVALID_ALLOWLIST_ADDRESS));

          let (found, _) = vector::index_of(&dest_chain_config.allowed_senders, &sender_address);
          if (!found) {
            vector::push_back(&mut dest_chain_config.allowed_senders, sender_address);
          };
        });
      ...
      if (vector::length(&remove_allowed_senders) > 0) {
        vector::for_each_ref(&remove_allowed_senders, |sender_address| {
          let (found, i) = vector::index_of(&dest_chain_config.allowed_senders, sender_address);
          if (found) {
            vector::swap_remove(&mut dest_chain_config.allowed_senders, i);
          }
        });
  ...
  inline fun calculate_metadata_hash(source_chain_selector: u64, dest_chain_selector: u64): vector<u8> {
    let packed = vector[];
    eth_abi::encode_bytes32(&mut packed, aptos_hash::keccak256(b"Aptos2AnyMessageHashV1"));
  ...
  inline fun calculate_message_hash(message: &Aptos2AnyRampMessage, metadata_hash: vector<u8>): vector<u8> {
    let outer_hash = vector[];
    eth_abi::encode_bytes32(&mut outer_hash, merkle_multi_proof::leaf_domain_separator());
    ...
    eth_abi::encode_bytes32(&mut outer_hash, aptos_hash::keccak256(message.receiver));
    eth_abi::encode_bytes32(&mut outer_hash, aptos_hash::keccak256(message.data));

    let token_hash = vector[];
    eth_abi::encode_u256(&mut token_hash, vector::length(&message.token_amounts) as u256);
    vector::for_each_ref(&message.token_amounts, |token_transfer| {
      let token_transfer: &Aptos2AnyTokenTransfer = token_transfer;
      eth_abi::encode_address(&mut token_hash, token_transfer.source_pool_address);
    ...
    eth_abi::encode_bytes32(&mut outer_hash, aptos_hash::keccak256(message.extra_args));

    aptos_hash::keccak256(outer_hash)
  }

  inline fun apply_dest_chain_config_updates_unchecked(dest_chain_selectors: vector<u64>, dest_chain_enabled: vector<bool>, dest_chain_allowlist_enabled: vector<bool>) {
    let dest_chains_len = vector::length(&dest_chain_selectors);
    assert!(dest_chains_len == vector::length(&dest_chain_enabled), error::invalid_argument(E_DEST_CHAIN_ARGUMENT_MISMATCH));
    assert!(dest_chains_len == vector::length(&dest_chain_allowlist_enabled), error::invalid_argument(E_DEST_CHAIN_ARGUMENT_MISMATCH));
    ...
    while (i < dest_chains_len) {
      let dest_chain_selector = *vector::borrow(&dest_chain_selectors, i);
      assert!(dest_chain_selector != 0, error::invalid_argument(E_INVALID_DEST_CHAIN_SELECTOR));

      let is_enabled = *vector::borrow(&dest_chain_enabled, i);
      let allowlist_enabled = *vector::borrow(&dest_chain_allowlist_enabled, i);

      let dest_chain_config = smart_table::borrow_mut_with_default(&mut state.dest_chain_configs, dest_chain_selector, DestChainConfig {
        is_enabled: false,
        sequence_number: 0,
