module ccip::ownable {
  ...
  public fun new(new_owner: &signer, pending_owner: address): OwnableState {
    ...
    if (pending_owner != @0x0) {
      transfer_ownership(signer::address_of(new_owner), &mut new_state, pending_owner);
    };
  ...
  public fun transfer_ownership(caller: address, state: &mut OwnableState, to: address) {
    ...
    event::emit_event(&mut state.ownership_transfer_requested_events, OwnershipTransferRequested {
      from: state.owner,
      to,
    });
  }
  ...
  public fun accept_ownership(caller: address, state: &mut OwnableState) {
    ...
    event::emit_event(&mut state.ownership_transferred_events, OwnershipTransferred {
      from: previous_owner,
      to: state.owner,
    });
  }
