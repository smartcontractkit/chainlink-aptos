module ccip::merkle_multi_proof {
  ...
  public fun merkle_root(
    ...
    assert!(leaves_len <= (MAX_NUM_HASHES + 1) && proofs_len <= (MAX_NUM_HASHES + 1), error::invalid_argument(E_INVALID_PROOF));
    ...
    while (i < total_hashes) {
      ...
      if ((proof_flag_bits & current_bit) == current_bit) {
        if (leaf_pos < leaves_len) {
          ...
          assert!(hash_pos < vector::length(&hashes), error::invalid_argument(E_INVALID_PROOF));
    ...
    assert!(hash_pos == (total_hashes - 1) && leaf_pos == leaves_len && proof_pos == proofs_len, error::invalid_argument(E_INVALID_PROOF));
  ...
  fun vector_u8_gt(a: &vector<u8>, b: &vector<u8>): bool {
    ...
    assert!(len == vector::length(b), error::invalid_argument(E_VECTOR_LENGTH_MISMATCH));
