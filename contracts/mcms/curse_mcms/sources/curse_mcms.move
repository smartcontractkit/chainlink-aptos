...
module curse_mcms::curse_mcms {
    ...
    use curse_mcms::bcs_stream::{Self, BCSStream};
    use curse_mcms::curse_mcms_account;
    use curse_mcms::params::{Self};
    ...
    const E_UNKNOWN_CURSE_MCMS_FUNCTION: u64 = 25;
    ...
    const E_UNKNOWN_CURSE_MCMS_TIMELOCK_FUNCTION: u64 = 48;
    ...
    inline fun create_multisig(publisher: &signer, role: u8): Object<Multisig> {
        ...
        move_to(
            ...
            Multisig {
                extend_ref,
                signers: ordered_map::new(),
                config: Config {
                    signers: vector[],
                    group_quorums: VEC_NUM_GROUPS,
                    group_parents: VEC_NUM_GROUPS
                },
                seen_signed_hashes: big_ordered_map::new_with_config(0, 0, false),
                expiring_root_and_op_count: ExpiringRootAndOpCount {
                    root: vector[],
                    valid_until: 0,
                    op_count: 0
    ...
    public entry fun set_root(
        ...
        assert!(
            !multisig.seen_signed_hashes.contains(&signed_hash),
            E_ALREADY_SEEN_HASH
        );
        ...
        assert!(metadata.multisig == @curse_mcms, E_WRONG_MULTISIG);
    ...
    public entry fun execute(
        ...
        assert!(op.multisig == @curse_mcms, E_WRONG_MULTISIG);
        ...
        assert!(
            op.to == @curse_mcms && *op.module_name.bytes() == b"curse_mcms",
    ...
    inline fun dispatch_to_timelock(
        ...
        if (function_name_bytes == b"timelock_schedule_batch") {
            ...
            abort E_UNKNOWN_CURSE_MCMS_TIMELOCK_FUNCTION
        }
    }
    ...
    public entry fun set_config(
        ...
        curse_mcms_account::assert_is_owner(caller);
        ...
        multisig.signers = ordered_map::new();
    ...
    #[view]
    public fun seen_signed_hashes(multisig: Object<Multisig>): vector<vector<u8>> acquires Multisig {
        borrow_multisig(multisig).seen_signed_hashes.keys()
    }
    ...
    #[view]
    public fun signers(multisig: Object<Multisig>): vector<Signer> acquires Multisig {
        borrow_multisig(multisig).signers.values()
    }
    ...
    inline fun borrow_multisig(obj: Object<Multisig>): &Multisig {
        borrow_global<Multisig>(object::object_address(&obj))
    }

    inline fun borrow_multisig_mut(multisig: Object<Multisig>): &mut Multisig {
        borrow_global_mut<Multisig>(object::object_address(&multisig))
    }

    inline fun borrow(): &MultisigState {
        borrow_global<MultisigState>(@curse_mcms)
    }

    inline fun borrow_mut(): &mut MultisigState {
        borrow_global_mut<MultisigState>(@curse_mcms)
    }
    ...
    /// Dispatch to RMN Remote - ONLY allows curse/uncurse functions
    /// This is the scoped-down dispatch that replaces the full MCMS dispatch
    inline fun timelock_dispatch_to_rmn_remote(
        function_name: String, data: vector<u8>
    ) {
        let function_bytes = *function_name.bytes();
        let stream = bcs_stream::new(data);

        // Get signer for calling RMN Remote
        let object_signer = &curse_mcms_account::get_signer();

        if (function_bytes == b"curse") {
            let subject = bcs_stream::deserialize_vector_u8(&mut stream);
            bcs_stream::assert_is_consumed(&stream);
            rmn_remote::curse(object_signer, subject);
        } else if (function_bytes == b"uncurse") {
            let subject = bcs_stream::deserialize_vector_u8(&mut stream);
            bcs_stream::assert_is_consumed(&stream);
            rmn_remote::uncurse(object_signer, subject);
    ...
    inline fun borrow_mut_timelock(): &mut Timelock {
        borrow_global_mut<Timelock>(@curse_mcms)
    }
    ...
    #[test_only]
    public fun test_timelock_dispatch_to_rmn_remote(
        function_name: String, data: vector<u8>
    ) {
        timelock_dispatch_to_rmn_remote(function_name, data)
    }
