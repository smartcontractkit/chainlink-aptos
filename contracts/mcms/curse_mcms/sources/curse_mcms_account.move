/// This module manages the ownership of the CurseMCMS package.
module curse_mcms::curse_mcms_account {
    ...
    /// Transfers ownership back to the `@curse_mcms` address.
    public entry fun transfer_ownership_to_self(caller: &signer) acquires AccountState {
        transfer_ownership(caller, @curse_mcms);
    }
    ...
    #[view]
    /// Returns `true` if the module is self-owned (owned by `@curse_mcms`).
    public fun is_self_owned(): bool acquires AccountState {
        owner() == @curse_mcms
    }

    #[view]
    /// Returns the address of the CurseMCMS account signer.
    public fun get_address(): address acquires AccountState {
        account::get_signer_capability_address(&borrow_state().signer_cap)
    }
    ...
    inline fun borrow_state(): &AccountState {
        borrow_global<AccountState>(@curse_mcms)
    }

    inline fun borrow_state_mut(): &mut AccountState {
        borrow_global_mut<AccountState>(@curse_mcms)
    }

    #[test_only]
    public fun init_module_for_testing(publisher: &signer) {
        let test_signer_cap = account::create_test_signer_cap(@curse_mcms);
