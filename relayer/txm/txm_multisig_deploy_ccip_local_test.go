...
func scheduleAndExecuteOperations(
	...
	setRootId := enqueueSetRoot(t, logger, merkleTree, txm, rootMetadata, bypasserSigners,
...
func runDeployMCMSAndCCIPInChunks(t *testing.T, logger logger.Logger, rpcURL string, keystore loop.Keystore, accounts []Account) {
	...
	setupInitialConfigAsDeployer(t, logger, txm, role, bypasserSigners, deployerAddress, deployerPublicKeyHex, true, deployMcmsAddress)
	...
	scheduleAndExecuteOperations(t, logger, txm, allOperations, bypasserSigners, deployerAddress, deployerPublicKeyHex, role, deployChainIdBig, delay, salt)
	...
	scheduleAndExecuteOperations(t, logger, txm, allUpgradeOps, bypasserSigners,
