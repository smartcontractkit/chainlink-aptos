...
func (a *AptosTxm) EnqueueWithEntryFunction(transactionID string, txMetadata *commontypes.TxMeta, publicKey string, entryFunction *aptos.EntryFunction, simulateTx bool, expectedSimulationFailures ...ExpectedSimulationFailureRule) (string, error) {
...
func (a *AptosTxm) createRawTx(client aptos.AptosRpcClient, tx *AptosTx, nonce uint64) (*aptos.RawTransaction, error) {
	...
	if tx.Simulate {
		...
		if err == nil {
			...
			switch classifyExpectedSimulationFailure(err, tx.ExpectedSimulationFailureRules) {
			case expectedSimulationFailureAbortBeforeSubmit:
				return nil, &expectedSimulationFailureError{reason: err.Error()}
			case expectedSimulationFailureSubmitOnce:
				ctxLogger.Warnw("simulation classified Aptos write failure as terminal; submitting once to obtain onchain tx hash", "error", err)
				a.updateTransactionVmStatus(tx, err.Error())
			default:
...
func matchExpectedSimulationFailure(err error, expectedSimulationFailures []ExpectedSimulationFailureRule) bool {
	return classifyExpectedSimulationFailure(err, expectedSimulationFailures) == expectedSimulationFailureAbortBeforeSubmit
}

type expectedSimulationFailureHandling uint8

const (
	expectedSimulationFailureNoMatch expectedSimulationFailureHandling = iota
	expectedSimulationFailureAbortBeforeSubmit
	expectedSimulationFailureSubmitOnce
)

func classifyExpectedSimulationFailure(err error, expectedSimulationFailures []ExpectedSimulationFailureRule) expectedSimulationFailureHandling {
	if err == nil {
		return expectedSimulationFailureNoMatch
	}

	errMsg := err.Error()
	for _, expectedSimulationFailure := range expectedSimulationFailures {
		if expectedSimulationFailure.Kind == ExpectedSimulationFailureRuleKindAptosWriteTerminal {
			classification := failures.ClassifyWriteVmStatus(errMsg)
			if classification.Decision != failures.WriteFailureDecisionRetryable {
				return expectedSimulationFailureSubmitOnce
			}
			continue
		}
		if strings.Contains(errMsg, expectedSimulationFailure.ErrorContains) {
			return expectedSimulationFailureAbortBeforeSubmit
		}
	}
