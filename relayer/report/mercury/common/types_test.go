package common

import (
	"encoding/hex"
	"testing"

	"github.com/stretchr/testify/require"
)

func TestDecodeReport(t *testing.T) {
	// Hex-encoded Mercury report data (example)
	// Test case sourced from: https://github.com/smartcontractkit/chainlink-internal-integrations/blob/8c5f8bfdec5ddb4484048fd3c123786cd5ee5a29/aptos/contracts/data-feeds/sources/registry.move#L661
	encoded := ""

	// Decode the hex data
	decoded, err := hex.DecodeString(encoded)
	require.NoError(t, err)

	m, err := Decode(decoded)
	require.NoError(t, err)

	require.Equal(t, 3, GetReportType(m.FeedId))
}
